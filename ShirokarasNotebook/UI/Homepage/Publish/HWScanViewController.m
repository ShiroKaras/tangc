...
- (void)creatControl
{
    ...
    label.text = @"在电脑上访问www.tangcao.vip\n点击“发送文章按钮”";
    label.numberOfLines = 2;
    label.font = PINGFANG_ROUND_FONT_OF_SIZE(11);
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if ([metadataObjects count] > 0) {
        ...
        [self stopScanning];
        //请求登录
        [[[SKServiceManager sharedInstance] topicService] postLoginWithToken:[[metadataObjects firstObject] stringValue] callback:^(BOOL success, SKResponsePackage *response) {
            if (response.errcode==0) {
                [self showAlertWithTitle:@"" message:@"登录成功" sureHandler:nil cancelHandler:nil];
            }
        }];
...
- (void)drawLineForImageView:(UIImageView *)imageView
{
    ...
    //设置开始颜色
    const CGFloat *startColorComponents = CGColorGetComponents([[UIColor whiteColor] CGColor]);
    //设置结束颜色
